...
\begin{minipage}[t]{.49\textwidth}
	\(\begin{array}[t]{r l}
		...
		\\[5pt]
		\text{Diagonalizable}:& A = QDQ^{-1} \ \ \text{\scriptsize\(\left(\begin{gathered}
				\text{must be}\\[-4pt]
				\text{nondefective}
			\end{gathered}\right)\)}\\[5pt]
		\text{Hessenberg}:& \text{\scriptsize Triang. but with 0s}\\[5pt]
		\text{Jordan Form}:& \text{\scriptsize Near diag. w/ entries in diag. above}
	\end{array}\)
\end{minipage}
% notes
\begin{minipage}[t]{.49\textwidth}
	\(\begin{aligned}[t]
		...
		&\bullet\ A(Tx) = \lambda (Tx) \ \rightarrow \ A \sim B = T^{-1}AT\\[5pt]
...
\begin{minipage}[t]{.49\textwidth}
	...
	\(
		...
		\end{gathered}: \ \boxed{PAP^T = R} 
		\hspace{20pt} 
		\begin{gathered}
			\text{\scriptsize(\(P\) is permu.)}\\[-7pt]
	...
	\(
		\begin{aligned}
			...
			&\bullet\ R_{ii} = \begin{cases} 
				...
				\end{cases}\\[5pt]
			&\bullet\ \text{\scriptsize All other entries are real}
		\end{aligned}
	\)
\end{minipage}
