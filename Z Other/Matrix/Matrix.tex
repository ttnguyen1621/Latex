...
\underline{Motivation}: \ \(
	...
	\begin{aligned}
		...
		&\downarrow\\[-7pt]
		Rx &= b_1' \ , \ \ r' = \text{\scriptsize\(\left(\begin{matrix}
				0 \\ 
				b_2'
			\end{matrix}\right)\)} 
...
\begin{minipage}[t]{.49\textwidth}
	...
	\(\begin{gathered}
		...
		\arraycolsep=3pt \begin{array}{r c l}
				H_1 a_1 &=& \alpha e_1 
					\hspace{15pt}
					\text{\scriptsize\(\left( \Vert a_1 \Vert = \vert \alpha \vert \right)\)}
					\\[10pt]
				H_i\dots H_1 a_i &=& \begin{gathered}[t]
					{\scriptstyle \sum}^i_j\ c_j e_j \ =\ H_n\dots H_1 a_i\\[5pt]
					\text{\scriptsize\(\left( \Vert a_i \Vert^2 = \vert \alpha \vert^2 
...
\end{minipage}
		...
		% 	H_n\dots H_1 A &= \text{\scriptsize\(\left(\begin{matrix}
		% 			R \\ 
		% 			0
		% 		\end{matrix}\right)\)}\\[5pt]
		% 	H_n\dots H_1 r &= \text{\scriptsize\(\left(\begin{matrix}
		% 			0 \\ 
		% 			r'
		% 		\end{matrix}\right)\)}\\[5pt]
		% 	H_n\dots H_1 b &= b'
...
%---------------------------------------------------------------		
\newpage
\subsubsection{Householder Transformation/Elementary Reflector}
\(\begin{aligned}
