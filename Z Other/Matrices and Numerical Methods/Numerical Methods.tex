...
\section{[Polynomial] Interpolation, \(f(t_i) = \sum_j x_j \hs \phi_j(t_i) = \hsvec{\phi}\hs(t_i) \cdot \hsvec{x}\)}
...
\(\begin{aligned}
    &\bullet\ \parbox[t]{.95\textwidth}{\scriptsize Runge Phenom.: As \(n\) increases, evenly-spaced \(t_i\) could produce a 
        high-dimensional polynomial \(f(t)\) that tends to be extremely wavey near the endpoints (like Gibbs phenom.).
...
\[
    ...
    \begin{array}{l c c c l c l c l c l}
        f_n(t) &=& f(t_0) 
            ...
            &+& \dots &+& \tfrac{ f^{(n)}(t_0) }{n!} (t-t_0)^n\\[5pt]
        f_n(t+h) &=& f(t) 
            &+& f'(t) h 
            &+& \tfrac{ f''(t) }{2}\hs h^2 
            &+& \dots 
...
\begin{minipage}{.45\textwidth}
    ...
    \(\begin{aligned}
        ...
        &\bullet\ \text{\scriptsize Finding \(f(t)\) from \(w_j\)'s is \(\mathcal{O}(n)\) work.}\\
        &\bullet\ \boxed{ \parbox[t]{1\textwidth}{\scriptsize Updating with an extra point \((t_{n+1}, y_{n+1})\) 
        ...
        &\bullet\ \boxed{ \int_{t_1}^{t_n} f(t) dt = \sum_{i=1}^n y_i \int_{t_1}^{t_n} \phi_i(t) dt }
    \end{aligned}\)
\end{minipage}    
...
\begin{minipage}[t]{.5\textwidth}
    ...
    \vspace{5pt}
    \(f_{n+1}(t) = f_n(t) + x_{n+1}\hs \phi_{n+1}(t)\)

    \vspace{10pt}
    \(
        \begin{aligned}
            &\begin{aligned}
                y_{n+1} &= f_{n+1}(t_{n+1})\\
                &= f_n(t_{n+1}) + x_{n+1}\hs \phi_{n+1}(t_{n+1})
            \end{aligned}
                \\[5pt]
...
\begin{minipage}[t]{.49\textwidth}
    ...
    \(
        ...
        \begin{aligned}
            ...
            &\bullet\ \parbox[t]{2cm}{\scriptsize Less prone to over/underflow.}
        \end{aligned}
    \)
\end{minipage}
