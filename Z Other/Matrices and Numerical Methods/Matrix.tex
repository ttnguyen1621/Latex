...
\begin{minipage}[t]{.50\textwidth}
	% Solving for f(x)
	\underline{Func./Vec. Condition Number (Solve for \(y\))}:

	\vspace{15pt}
	\(\begin{aligned}
		% Rel. Cond. Number
		&\begin{aligned}
				\text{[rel.]}\ \text{cond}_f(x) 
				&= \left\vert \frac{\Delta y / y}{\Delta x / x}\right\vert 
					= \left\vert \tfrac{\text{Rel. Forward Err.}}{\text{Rel. Backward Err.}} \right\vert
					\\[3pt]
				&= \boxed{ \left\vert \tfrac{[f(\hat{x}) - f(x)] / f(x)}{[\hat{x} - x] / x} \right\vert	}
					\hspace{15pt}\text{\scriptsize(vector)}
					\\[3pt]
				&= \boxed{ \left\vert \tfrac{x f'(x)}{f(x)}\right\vert }
					\hspace{15pt}\text{\scriptsize(func.)}
			\end{aligned}
			\\[13pt]
		% Abs Cond. Number
		&\begin{aligned}
				\begin{gathered}[t]
						\text{[abs.]}\ \text{cond}_f(\hat{x}) \\[3pt]
						\left( \begin{gathered}
								\text{\scriptsize use when \(f(x) = 0\)}\\[-7pt]
								\text{\scriptsize or \(x=0\)}
							\end{gathered} \right)
					\end{gathered} \ 
				&= \left\vert \frac{\Delta y}{\Delta x}\right\vert 
					= \left\vert \tfrac{\text{Abs. Forward Err.}}{\text{Abs. Backward Err.}} \right\vert
					\\[-20pt]
				&= \boxed{ \left\vert f'(\hat{x}) \right\vert }
			\end{aligned}
	\end{aligned}\)

	% Solving for x
	\vspace{30pt}
	\underline{Inverse Condition Number (Solve for \(x\))}:

	\vspace{15pt}
	\(\begin{aligned}
		\boxed{ \text{cond}_{f^{-1}}(x) \ =\ 1 / \text{cond}_f(x) }
	\end{aligned}\)
\end{minipage}
...
\vspace{5pt}
\underline{Positive Definite}:\ \ \(\boxed{x^T Ax > 0} 
	\ \Leftrightarrow\ \boxed{ \lambda > 0 }
	\hspace{10pt} \text{\scriptsize\((Ax = \lambda x)\)} \) \\[15pt]
...
% Assumptions
\(\begin{aligned}[t]
	&\bullet\ A = A_{m \times n} \hspace{10pt} \boxed{ \text{\scriptsize\((m > n\),\ \ overdetermined\()\)} }\\[5pt]
...
\underline{Motivation}: \ \(
	...
	\ \ \rightarrow \ \
	\begin{aligned}
		\Vert r \Vert^2 & = \Vert r' \Vert^2 = \cancel{ \Vert b_1' - Rx \Vert^2 } + \Vert b_2' \Vert^2 \\
		& \downarrow\\
	...
	&\bullet\ 
		\begin{aligned}[t]
			\alpha_1 e_1 = a_1 - (2v_1)\frac{v_1 \cdot a_1}{v_1 \cdot v_1} 
				&\ \ \Rightarrow\ \ v_1 = (a_1 - \alpha_1 e_1) \cancel{ \frac{v_1 \cdot v_1}{2v_1 \cdot a_1} } 
				\hspace{15pt} \text{\scriptsize(magnitude doesn't matter)}
				\\[5pt]
			&\ \ \rightarrow\ \ \boxed{ v_1 = (a_1 - \alpha_1 e_1) }
				\\[10pt]
			\alpha_1 = \pm \Vert a_1 \Vert
				& \ \ \rightarrow\ \ \underline{ \alpha_1 = - \text{sign}(a_1 \cdot e_1) \Vert a_1 \Vert }
				\hspace{15pt} \text{\scriptsize(avoid "cancellation" in finite-calc. of \(v\) above)}
				\\[5pt]
			\begin{gathered}
				H_j\dots H_1 a_i = a_i^j \\
				(a_i^j)' = \mss{ \left[ (a_i^j)_i, (a_i^j)_{i+1}, ..., (a_i^j)_m \right]^T }
			\end{gathered}	
				& \ \ \rightarrow\ \ \boxed{ 
					v_{i} 
					= 
					\mss{ \left[ \begin{matrix}
						0 \leftarrow {\scriptstyle i > 1 }\\[3pt]
						(a_i^{i-1})'\\
					\end{matrix} \right] } 
					- \alpha_i e_i 
				}
				...
				\hspace{17pt}
				\begin{aligned}
					& \bullet\ \boxed{ \alpha_i = - \text{sign}(a_i^{i-1} \cdot e_i) \Vert (a_i^{i-1})' \Vert  }\\
...
\vspace{10pt}
\underline{Row Oriented}:\ \ For all \(1\leq i \leq n\), build each \(q_i\) from it's sum, term by term.
	\\[5pt]
% Notes
\(\hspace{10pt} \begin{aligned}
...
\begin{minipage}[t]{.49\textwidth}
	\(\begin{array}[t]{r l}
		...
		\text{Hermitian}:& H = H^\dagger = UDU^\dagger \hspace{10pt} \text{\scriptsize(\(D\) is real)}\\[5pt]
		\text{Jacobian}:& J_f = ( \vec{\nabla} \otimes \vec{f}\ )^T = ( \vec{\nabla} \vec{f\ }^T )^T
	\end{array}\)
\end{minipage}
\hfill
\begin{minipage}[t]{.49\textwidth}
	\(\begin{array}[t]{r l}
		...
		\text{Jordan Form}:& \parbox[t]{4cm}{\scriptsize Nonsing. trans. into a near diag. w/ entries in diag. above}\\[15pt]
		\text{Hessian}:& H_f = \vec{\nabla} \otimes (\vec{\nabla} f) = \vec{\nabla} (\vec{\nabla} f)^T
	\end{array}\)
\end{minipage}
...
\vfill
\subsection{Jacobi Method (when \(A=A^T\))}
\(\begin{aligned}
	&J \text{ is a Givens Rotation Matrix}\\[5pt]
	&\boxed{ A_{k+1} = J^T A_k J 
		= J^T \text{\scriptsize\(\left(\begin{matrix}
			a & b\\
			b & d
		\end{matrix}\right)\)}
		\text{\scriptsize\(\left(\begin{matrix}
			...
			-s & c
		\end{matrix}\right)\)} } \\
	&\boxed{ 0 = 1 + t \cdot \tfrac{(a-d)}{b} - t^2 }
		\hspace{10pt} \text{\scriptsize(Choose smaller \(\left\vert t=\tfrac{s}{c} \right\vert\))}
\end{aligned}\)
% Notes
\hfill
\(\begin{aligned}
	&\bullet\ \text{\scriptsize Creates 0s by rotating the column vectors and then the row vectors.}\\[-3pt]
	&\bullet\ \text{\scriptsize The row vector rotation doesn't preserve 0s at all.}\\[-3pt]
	&\bullet\ \text{\scriptsize\(\lim_{k\rightarrow\infty} A_k =\) Diag.}\\
	&\bullet\ \text{\scriptsize Each matrix sweep costs the same as tridiag. + QR}\\[-3pt]
	&\bullet\ \text{\scriptsize Easy to use with parallel-computing.}
\end{aligned}\)
