...
\parbox{.35\textwidth}{\scriptsize 
	(proof?) Single charges do not constitute a current. Free charges (unaffected by external forces holding them) %
	will always, by self-interaction through their E-field, create an unconstant current and thus unconstant B-field. %
	Under static conditions, a free space must have a \(\rho = 0\).%
}
...
\(\begin{aligned}
	...
	& \hspace{10pt} \begin{aligned}[t]
			...
			& \begin{aligned}
					& \big[ 
							\underline{ \vec{E}_{!m\perp} \mss{(\hsvec{b}\hs)} = 0 }
				...
				\\[5pt]
			% Circuit Wire case
			& \vec{E}_{!m} \mss{(\hsvec{r}_m)} \neq 0 \hspace{5pt} (\text{\scriptsize circuit wire \(J = J_0\)})
				\ \Rightarrow\ \nabla^2 E = \tfrac{\nabla \rho}{\epsilon_0}
		\end{aligned}
\end{aligned}\)
...
\begin{minipage}[t]{0.45\textwidth}
	...
	\vspace{10pt}
	If \(\boldsymbol{B = \nabla \times A = \nabla \times (A' + \nabla \lambda) = 0 }\), then \\[5pt]
	let \(A' = - \nabla \lambda \ \Leftrightarrow\ \lambda = - \int A' \cdot dl\)

	\vspace{10pt}\hspace{5pt}
	\(
		\begin{aligned}[t]
			&\bullet\ A = 0 \rightarrow \tfrac{\partial A}{\partial t} = 0\\
			&\bullet\ E = -\nabla( V -\tfrac{\partial \lambda}{\partial t}) - \tfrac{\partial A'}{\partial t} = - \nabla V
		\end{aligned}
	\)
